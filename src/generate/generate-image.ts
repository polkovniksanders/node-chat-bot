const BASE_URL = process.env.IMAGE_API_URL ?? process.env.POLZA_API_URL ?? 'https://api.polza.ai/v1';
const API_KEY = process.env.IMAGE_API_KEY ?? process.env.POLZA_API_KEY ?? '';
...
async function apiPost(path: string, body: Record<string, unknown>): Promise<any> {
  ...
  if (!res.ok) throw new Error(`Image API ${path} error ${res.status}: ${text}`);
...
async function createTask(model: string, prompt: string): Promise<string> {
  ...
  if (!taskId) throw new Error(`Image API не вернул task ID: ${JSON.stringify(data)}`);
  return taskId;
}
...
async function pollResult(taskId: string): Promise<string> {
  ...
  while (Date.now() < deadline) {
    ...
    if (data?.status === 'done') {
      ...
      if (!url) throw new Error('Image API вернул status=done, но URL отсутствует');
      return url;
    }

    if (data?.status === 'error') {
      throw new Error(`Image API вернул ошибку генерации: ${JSON.stringify(data)}`);
    }

    // status: idle | processing — продолжаем ждать
  }

  throw new Error('Image API: таймаут генерации изображения (120 сек.)');
}

async function tryGenerateWithModel(model: string, prompt: string): Promise<string> {
  const taskId = await createTask(model, prompt);
  console.log(`🎨 Image generation task created (${model}): ${taskId}`);
